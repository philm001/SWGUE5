...
#include "MeshMergeLibrary.h"
#include "SkeletalMeshMerge.h"
#include "Engine/SkeletalMeshSocket.h"
#include "Engine/SkeletalMesh.h"
#include "Animation/Skeleton.h"

UMeshMergeLibrary::UMeshMergeLibrary()
{
}
...
static void ToMergeParams(const TArray<FSkelMeshMergeUVTransformMapping>& InUVTransformsPerMesh, TArray<FSkelMeshMergeUVTransforms>& OutUVTransformsPerMesh)
{
    if (InUVTransformsPerMesh.Num() > 0)
    {
        ...
        for (int32 i = 0; i < InUVTransformsPerMesh.Num(); ++i)
        {
            ...
            //const TArray<FSkelMeshMergeMeshUVTransforms>& InUVTransforms = InUVTransformsPerMesh[i].UVTransformsPerMesh;// Edit made here
            if (InUVTransformsPerMesh[i].UVTransformsPerMesh.Num() > 0)
            {
                OutUVTransforms.Empty();
                OutUVTransforms.AddUninitialized(InUVTransformsPerMesh[i].UVTransformsPerMesh.Num());
                for (int32 j = 0; j < InUVTransformsPerMesh[i].UVTransformsPerMesh.Num(); j++)
                {
                    OutUVTransforms[i] = InUVTransformsPerMesh[i].UVTransformsPerMesh[i].UVTransforms;
                }
            }
        }
    }
...
USkeletalMesh* UMeshMergeLibrary::MergeMeshes(const FSkeletalMeshMergeParams& Params)
{
